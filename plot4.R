...
##先format date
data.sel$datetime.format <- strptime(data.sel$datetime, "%d/%m/%Y %H:%M:%S")
##使用英文字
...
par(mfrow = c(2, 2))
### plot4-1
plot(data.sel$datetime.format, data.sel$Global_active_power, type ="l")
### plot 4-2
plot(data.sel$datetime.format, data.sel$Voltage, type ="l")
### Plot 4-3
plot(data.sel$datetime.format, data.sel[,7], type ="n")
lines(data.sel$datetime.format, data.sel[,7])
lines(data.sel$datetime.format, data.sel[,8], col ="red")
lines(data.sel$datetime.format, data.sel[,9], col ="blue")
### plot 4-4
plot(data.sel$datetime.format, data.sel$Global_reactive_power, type ="l")
dev.off()
